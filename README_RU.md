...
В файле `config.json` для каждого аккаунта добавьте следующие поля:
...
1. **Без аутентификации:** `host:port`
   ```json
   "proxy_url": "192.168.1.100:1080"
   ```

2. **С аутентификацией:** `host:port:username:password`
   ```json
...
- `proxy_url` (string) - адрес прокси в указанном формате
